...
int main(int argc, char ** argv)
{
	...
	SDL_Surface *deck[13],*objet[8],*gobutton,*connectbutton,*logo_su,*logo_ps,*play_again; /*declaration des surfaces SDL_Surface 
	...
	play_again = IMG_Load("rejouer.png"); // load bouton relancer
	// transforme les surfaces en textures pour l'affichage
	SDL_Texture *texture_deck[13],*texture_gobutton,*texture_connectbutton,*texture_objet[8],*texture_logo_su,*texture_logo_ps,*texture_play_again;
	...
	texture_play_again = SDL_CreateTextureFromSurface(renderer, play_again); // transforme les surfaces en textures pour l'affichage
	...
	while (rejouer)
	{
		...
		while (!quit) // boucle principale SDL 
		{
			if (SDL_PollEvent(&event))
			{
				...
				switch (event.type) // traite les evenements
				{
					...
					case  SDL_MOUSEBUTTONDOWN: // un clic de souris
						...
						else if (mx >= 580 && mx < 880 && my >= 650 && my < 714 && vainqueur != -1) // clic sur le bouton rejouer
						{
							rejouer = 1;
							quit = 1;
						}
			...
			} else 
			{
				...
				// Affichage bouton relancer
				largeur_voulue = 300;
				SDL_QueryTexture(texture_play_again, NULL, NULL, &w_tex, &h_tex);
				int hauteur_auto_play_again = (largeur_voulue * h_tex) / w_tex;
				SDL_Rect dstrect = { 580, 650, largeur_voulue, hauteur_auto_play_again }; // {x, y, w, h}
				SDL_RenderCopy(renderer, texture_play_again, NULL, &dstrect);
			}
